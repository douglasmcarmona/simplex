...
public class teste {
	...
	public static float[][] m_resposta;
	public static String resposta;
	private static String[] vetorLinha;
	private static String[] vetorColuna;
	/*
	 * Construtor para pegar o vetor passado como parametro Salva o vetor
	 * passado para ser inserido na matriz
	 */
	...
	/*
	 * Preenche a minha matrizEquacao com os valores que estao nas minhas
	 * equacoes
	 */
	public float[][] CriarMatriz() {
		String s = "";
		String aux = "";
		String aux1 = "";
		...
		for (int j = 1; j < s.length(); j++) {
			if (s.charAt(j - 1) != 'x') {
				if (s.charAt(j) >= '0' && s.charAt(j) <= '9') {
					contNum++;
					aux = aux.concat("" + s.charAt(j));// concatena cada parte
														// do meu numero ate
														// encontrar algo
														// diferente de um
														// numero
		...
		for (int i = 1; i < vetor.length; i++) {
			...
			for (int j = 4; j < s.length(); j++) {
				if (s.charAt(j - 1) != 'x') { // Verifica se a posicao anterior
												// e diferente de x
					if (s.charAt(j) >= '0' && s.charAt(j) <= '9') {
						contNum++;
						aux = aux.concat("" + s.charAt(j));// concatena cada
															// parte do meu
															// numero ate
															// encontrar algo
															// diferente de um
						...
						if (contNum == 1) {
							if (s.charAt(j - 1) >= '0' && s.charAt(j - 1) <= '9') {
								// Verifica se o o sina lque esta na frente do
								// numero e + ou -
								if (s.charAt(j - (contNum + 1)) == '+') {
									convert = Float.parseFloat(aux);
									matrizEquacao[linha][coluna] = convert;
					...
					// Salva na matriz os valores 1 quando nao existe um numero
					// na frente do X de acordo com o sinal que o precede
		...
		return MetodoFase1(matrizEquacao);
	}

	private static float[][] MetodoFase1(float[][] matriz) {
		...
		do {
			...
			if (linha != true)
				 return MetodoFase2(matriz);
			else {
				// Preenche o vetor com a menor variavel para verificacao de
				// menor quociente
				...
				for (int m = 1; m < Simplex.quantEquacoes + 1; m++) {
					if ((matriz[m][MinColuna] < 0 && matriz[m][0] < 0)
							|| (matriz[m][MinColuna] > 0 && matriz[m][0] > 0)) {
				...
				for (int j = 0; j < Simplex.quantTipoDesenho + 1; j++) {
					if (j != MinColuna) {
						matrizTransformada[posicaoQuociente
								+ 1][j] = matrizTransformada[posicaoQuociente + 1][MinColuna]
										* matriz[posicaoQuociente + 1][j];
					}
				}
				// Multiplicacao da coluna pelo valor inverso negativo
				matrizTransformada[posicaoQuociente
						+ 1][MinColuna] = (matrizTransformada[posicaoQuociente + 1][MinColuna]) * (-1);
				for (int i = 0; i < Simplex.quantEquacoes + 1; i++) {
					if (i != posicaoQuociente + 1) {
						matrizTransformada[i][MinColuna] = matrizTransformada[posicaoQuociente + 1][MinColuna]
								* matriz[i][MinColuna];
					}
				}
						...
						while (j < Simplex.quantEquacoes + 1) {
							if (matrizTransformada[j][i] == Float.MIN_VALUE) {
								if (j != posicaoQuociente + 1 && i != MinColuna) {
									aux = matriz[posicaoQuociente + 1][PercorreLinha]
											* matrizTransformada[j][MinColuna];
									aux = matriz[j][i] + aux;
				...
				for (int linha1 = 0; linha1 < Simplex.quantEquacoes + 1; linha1++) {
					for (int coluna1 = 0; coluna1 < Simplex.quantTipoDesenho + 1; coluna1++) {
						if (matrizTransformada[linha1][coluna1] == -0)
							matrizTransformada[linha1][coluna1] = 0;
					}
				}
		...
		return MetodoFase2(matrizTransformada);
		
	}// FimMetodoFase1

	public static float[][] MetodoFase2(float matriz[][]) {
		...
		do {
			...
			if (resSO)
				...
				resposta = ("Solucao Impossivel");
			else if (resSM)
				resposta = ("Solucao Multipla");
			else if (resSIli)
				resposta = ("Solucao Ilimitada");
			else
				m = Teste(m);
	...
	public static boolean SolucaoIlimitada(float matriz[][]) {
		...
		if (linha2 && contNegativo == Simplex.quantEquacoes)
			SIli = true;
	...
	public static boolean SolucaoMultipla(float matriz[][]) {
		...
		for (int coluna1 = 1; coluna1 < Simplex.quantTipoDesenho + 1; coluna1++) {
			if (matriz[0][coluna1] == 0)
				encontrou0 = true;
			else if (matriz[0][coluna1] < 0)
				encontrouNeg = true;
		}

		if (contPositivo == Simplex.quantEquacoes && encontrou0 && encontrouNeg)
			MS = true;
	...
	public static boolean SolucaoOtima(float matriz[][]) {
		...
		if (contNegativo == Simplex.quantTipoDesenho && contPositivo == Simplex.quantEquacoes)
			SoOt = true;
	...
	public static float[][] Teste(float matriz[][]) {
		...
		TrocaVariaveis(MinLinha, MinColuna);
		// Preenche o vetor com a menor variavel para verificacao de menor
		// quociente
		...
		for (int j = 0; j < Simplex.quantTipoDesenho + 1; j++) {
			if (j != MinColuna) {
				matrizTransformada1[posicaoQuociente][j] = matrizTransformada1[posicaoQuociente][MinColuna]
						* matriz[posicaoQuociente][j];
			}
		}
		// Multiplicacao da coluna pelo valor inverso negativo
		...
		for (int i = 0; i < Simplex.quantEquacoes + 1; i++) {
			if (i != posicaoQuociente) {
				matrizTransformada1[i][MinColuna] = matrizTransformada1[posicaoQuociente][MinColuna]
						* matriz[i][MinColuna];
			}
		}
		// Preenchimento do resto da matriz
		...
		for (int linha1 = 0; linha1 < Simplex.quantEquacoes + 1; linha1++) {
			for (int coluna1 = 0; coluna1 < Simplex.quantTipoDesenho + 1; coluna1++) {
				if (matrizTransformada1[linha1][coluna1] == -0)
					matrizTransformada1[linha1][coluna1] = 0;
			}
		}

		matrizTransformada1[posicaoQuociente + 1][MinColuna] = (matrizTransformada1[posicaoQuociente + 1][MinColuna])
				* (-1);
